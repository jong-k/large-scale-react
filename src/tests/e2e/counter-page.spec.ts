...
test.describe("Counter Page Tests", () => {
  ...
  test("should navigate from root route to /counter route", async ({
    page,
  }) => {
    // 루트 페이지에서 카운터 링크 확인
    ...
    await expect(
      page.locator("h2").filter({ hasText: "Counter List" })
    ).toBeVisible();
  });
  ...
  test("should increase counter value when + button is clicked", async ({
    page,
  }) => {
    await page.goto("http://localhost:5173/counter");
  ...
  test("should decrease counter value when - button is clicked", async ({
    page,
  }) => {
    await page.goto("http://localhost:5173/counter");

    // 두 번째 카운터의 - 버튼 클릭 (초기값 10)
    const secondCounterDiv = page
      .locator("h3")
      .filter({ hasText: "테스트 카운터2" })
      .locator("..");
    const secondCounterElement = secondCounterDiv.locator("span");
    await expect(secondCounterElement).toHaveText("10");

    const secondMinusButton = secondCounterDiv
      .locator("button")
      .filter({ hasText: "-" });
  ...
  test("should handle multiple counter interactions", async ({ page }) => {
    ...
    const firstPlusButton = firstCounterDiv
      .locator("button")
      .filter({ hasText: "+" });
    const firstMinusButton = firstCounterDiv
      .locator("button")
      .filter({ hasText: "-" });
